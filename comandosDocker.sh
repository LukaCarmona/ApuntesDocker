...
# mysql:
docker run -dp 3306:3306 --name nombre-sql -v /${PWD}/mysql:/var/lib/mysql --network nombre-red -e MYSQL_ROOT_PASSWORD="123" mysql
...
# php-apache:
docker run -dp 80:80 --name nombre-apache -v /${PWD}/apache:/var/www/html --network nombre-red $(docker build . -q)
